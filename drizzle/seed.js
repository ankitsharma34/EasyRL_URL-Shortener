...
const USER_ID = 5; // choose userId
await reset(db, { shortLinksTable: schemas.shortLinkTable });
await seed(
  db,
  { shortLinksTable: schemas.shortLinkTable },
  { count: 100 },
).refine((f) => ({
  shortLinksTable: {
    columns: {
      userId: f.default({ defaultValue: USER_ID }),
      url: f.default({ defaultValue: "https://picsum.photos/200/300" }),
    },
  },
}));
process.exit(0);
